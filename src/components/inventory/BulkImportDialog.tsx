...
const TEMPLATE_HEADERS = [
  ...
  "Price 1", "Price 2", "Price 3", "Cost Price", 
  "Min Qty", "Initial Qty"
]
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (!firstRow.Name && !firstRow.name && !firstRow['الاسم']) {
          toast.error("تنسيق الملف غير صحيح، يجب أن يحتوي على عمود 'Name'")
          return
        }
        ...
        const normalizedData: ImportData[] = parsedData.map(row => ({
          ...
          Barcode: row.Barcode || row.barcode || row['الباركود']?.toString() || '',
          SKU: row.SKU || row.sku || row['SKU']?.toString() || '',
          Category: row.Category || row.category || row['الفئة'] || row['القسم'] || '',
          Unit: row.Unit || row.unit || row['الوحدة'] || '',
          "Price 1": row['Price 1'] || row.price1 || row['السعر الأساسي'] || row['السعر'] || 0,
          "Price 2": row['Price 2'] || row.price2 || row['سعر الجملة'] || 0,
          "Price 3": row['Price 3'] || row.price3 || 0,
