...
import { Pause, RotateCcw, ArrowRight, Moon, Sun } from "lucide-react"
  ...
  return (
    <header className="h-16 bg-white dark:bg-slate-900 border-b flex items-center justify-between px-4 shadow-sm z-50">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild aria-label="العودة إلى لوحة التحكم">
          <Link href="/dashboard">
            <ArrowRight className="h-5 w-5" aria-hidden />
          </Link>
        </Button>
        <div className="flex flex-col">
          <h1 className="text-lg font-bold leading-tight text-primary">نقطة البيع</h1>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" size="sm" onClick={holdCart} className="gap-2 border-orange-200 bg-orange-50 text-orange-600 hover:bg-orange-100 dark:bg-orange-950/20">
          <Pause className="h-4 w-4 shrink-0" aria-hidden />
          تعليق سريع
        </Button>

        <Button variant="outline" size="sm" onClick={clearCart} className="gap-2 border-red-200 bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-950/20">
      ...
      <div className="flex items-center gap-4">
        ...
        <Button variant="ghost" size="icon" className="relative rounded-full" type="button" aria-label="تبديل الوضع الفاتح أو الداكن">
