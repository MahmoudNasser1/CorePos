import { getPartnerStatement, getSupplierById } from "@/lib/actions/customers.actions"
...
export default async function SupplierDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const partner = await getSupplierById(id)
  ...
  return (
    <div className="space-y-6 rounded-2xl border border-amber-500/15 border-s-4 border-s-amber-500/45 bg-amber-50/15 p-4 md:p-6 dark:bg-amber-950/10">
      <PageHeader title={String(p.name ?? "مورد")} subtitle="بيانات المورد وكشف الحساب والروابط السريعة للمشتريات والسداد.">
        <Button type="button" variant="outline" size="sm" asChild>
          <Link href={`/dashboard/suppliers/${id}/edit`}>تعديل البيانات</Link>
        </Button>
      </PageHeader>
