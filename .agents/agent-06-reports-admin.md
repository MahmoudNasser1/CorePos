...
// الخزينة: تحديث عبر إعادة جلب البيانات (أو WS لاحقاً)
```
...
// Data source: backend reports endpoints
```
...
- شعار يُرفع إلى Storage service (S3-compatible) عبر backend
...
- إضافة مستخدم جديد → دعوة/إنشاء عبر backend (email optional حسب النظام)
- تعديل الدور والفرع
