...
### FIN-O8-001 — تدفق الخزينة ↔ السندات + أيقونة مصروفات
- **Severity**: 🟢 polish
- **Location**: `finance/treasuries/page.tsx`، `finance/payments/new/page.tsx`، `finance/receipts/new/page.tsx`، `finance/expenses/page.tsx`
- **Current**: مسارات قبض/صرف بلا إحالة سريعة للخزائن أو للمسار المقابل؛ زر «مصروف جديد» بأيقونة بلا `aria-hidden`.
- **Expected**: صناديق توضيح مع روابط متبادلة (خطة 08 — التدفقات 1–2)؛ اتساق وصولية الأيقونات.
- **Evidence**: `08-finance.md` (§3 تدفقات).
- **✅ Fixed**: نعم — روابط بين الخزائن وسندَي القبض/الصرف؛ روابط عكسية من صفحات السندات؛ `aria-hidden` على أيقونة زر المصروف.
...
### CON-O9-001 — أزرار إضافة عميل/مورد (RTL + صدق)
- **Severity**: 🟢 polish
- **Location**: `customers/page.tsx`، `suppliers/page.tsx`
- **Current**: `me-2` على الأيقونة داخل زر معطّل؛ بلا `title` يوضح سبب التعطيل.
- **Expected**: `gap-2` + `shrink-0`؛ تلميح عربي عند عدم توفر النموذج.
- **Evidence**: خطة `09-contacts.md` (T9.4، G4 A11y).
- **✅ Fixed**: نعم — `gap-2` و`title` توضيحي؛ `aria-hidden` على الأيقونات.

### REP-DSH-SET-HLP-O12-001 — استكمال مراحل 10 و 03 و 11 و 12
- **Severity**: 🟢 polish
- **Location**: التقارير، لوحة التحكم، الإعدادات/التسويق/الفوترة، المساعدة/التدقيق/سوبر أدمن
- **Current**: بنود الخطة مُغطاة مسبقاً في هذا الملف (`REP-T10…`، `DSH-T3…`، `SET-T11…`، `HLP-T12…`) دون حاجة لتعديل كود إضافي في هذه الدفعة.
- **Expected**: عدم إدخال تغييرات وهمية عند اكتمال المعايير.
- **Evidence**: مراجعة `docs/ui_ux_audit.md` مقابل `00-README.md` ترتيب التنفيذ.
- **✅ Fixed**: نعم — تأكيد حالة الإغلاق بالتوثيق؛ لا تغييرات كود جديدة مطلوبة لهذه المجموعة في هذه الجولة.
